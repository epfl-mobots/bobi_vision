...
v4l2-ctl -d /dev/video4 --set-fmt-video=width=640,height=480,pixelformat='YUYV'
v4l2-ctl -d /dev/video4 -c exposure_absolute=305

gst-launch-1.0 v4l2src device=/dev/video4 ! video/x-raw,framerate=30/1,width=640,height=480 ! videoflip method=horizontal-flip ! videoconvert ! tee ! v4l2sink device=/dev/video3 sync=false &
